...
def _format_gap_seconds(value):
    ...
    seconds, millis = divmod(remainder, 1000)
    if hours > 0:
        return f"{hours}:{minutes:02d}:{seconds:02d}.{millis:03d} s"
    if minutes > 0:
        return f"{minutes}:{seconds:02d}.{millis:03d} s"
...
def _session_track_status_segments(session, include_yellow=False):
    ...
    def classify_track_status(raw_status):
        ...
        if include_yellow and "2" in status:
    ...
    if track_status is None or track_status.empty or laps_for_status is None or laps_for_status.empty:
        return []
        ...
        options.append(
            {
                ...
                "headshot_url": headshot_url,
                "label": f"{abbreviation} - {full_name}" if abbreviation and full_name else full_name or abbreviation or driver_number,
                "result_time": _format_lap_time(driver.get("time_label", "-")),
            }
        )
...
def _qualifying_phase_label_for_lap(session, lap, phase_windows=None):
    if lap is None:
        return None

    session_name = str(getattr(session, "name", "")).strip().lower()
    phase_prefix = "SQ" if "sprint" in session_name else "Q"
    resolved_phase_windows = phase_windows if isinstance(phase_windows, dict) and phase_windows else _qualifying_phase_windows(session)
    ...
    q2_start = resolved_phase_windows.get("Q2")
    q3_start = resolved_phase_windows.get("Q3")
    try:
        lap_start_value = float(lap_start)
    except (TypeError, ValueError):
        return None
    ...
    try:
        q2_start_value = float(q2_start)
    except (TypeError, ValueError):
        q2_start_value = None

    if q3_start is None:
        if q2_start_value is not None and lap_start_value < q2_start_value:
            return f"{phase_prefix}1"
        return f"{phase_prefix}2"

    try:
        q3_start_value = float(q3_start)
    except (TypeError, ValueError):
        q3_start_value = None

    if q2_start_value is not None and lap_start_value < q2_start_value:
        return f"{phase_prefix}1"
    if q3_start_value is not None and lap_start_value < q3_start_value:
        return f"{phase_prefix}2"
    return f"{phase_prefix}3"
...
def _qualifying_driver_list_rows(phase_rows):
    if not phase_rows:
        return []
...
def _pit_stop_stationary_seconds(session, driver_number, previous_stint, current_stint, speed_threshold_kmh=0.0):
    if not session or not driver_number:
        return None
    ...
    previous_lap_number = previous_laps[-1].get("lap_number")
    current_lap_number = current_laps[0].get("lap_number")
    if previous_lap_number is None or current_lap_number is None:
        return None
    ...
    try:
        lap_subset = driver_laps.pick_laps([previous_lap_number, current_lap_number])
    except Exception:
        return None
    if lap_subset is None or lap_subset.empty:
        return None

    try:
        telemetry = lap_subset.get_car_data().reset_index(drop=True)
    except Exception:
        try:
            telemetry = lap_subset.get_telemetry(frequency="original").reset_index(drop=True)
        except Exception:
            return None
    ...
    try:
        frame[time_column] = pd.to_timedelta(frame[time_column])
    except Exception:
        return None
    ...
    window_start = previous_laps[-1].get("pit_in_seconds")
    if window_start is None:
        window_start = previous_laps[-1].get("lap_start_seconds")
    window_end = current_laps[0].get("pit_out_seconds")
    if window_end is None:
        window_end = current_laps[0].get("lap_start_seconds")
    if window_start is not None:
        frame = frame[frame[time_column] >= float(window_start) - 0.25]
    if window_end is not None:
        frame = frame[frame[time_column] <= float(window_end) + 0.25]
    ...
    for threshold in thresholds:
        measured = measure_stationary_seconds(threshold)
...
def _race_stints(session, driver_number, session_code):
    ...
    if driver_laps is not None and not driver_laps.empty:
        ...
        for index, stint in enumerate(stints):
            ...
            pit_stop_stopped_seconds = _pit_stop_stationary_seconds(
                session,
                stint.get("driver_number", driver_number),
                stints[index - 1],
                stint,
...
def _pit_strategy_graph(session, session_code=None):
    ...
    for driver in driver_meta:
        ...
        for stint_number, stint_laps in ordered.groupby("Stint", dropna=True):
            ...
            stint_start_candidates = []
            stint_end_candidates = []
            for _, stint_lap in stint_laps.iterrows():
                lap_start_seconds = _lap_time_seconds(stint_lap.get("LapStartTime", None))
                lap_end_seconds = _lap_time_seconds(stint_lap.get("Time", None))
                if lap_start_seconds is not None:
                    stint_start_candidates.append(float(lap_start_seconds))
                elif lap_end_seconds is not None:
                    stint_start_candidates.append(float(lap_end_seconds))
                if lap_end_seconds is not None:
                    stint_end_candidates.append(float(lap_end_seconds))
                elif lap_start_seconds is not None:
                    stint_end_candidates.append(float(lap_start_seconds))

            stint_start_seconds = min(stint_start_candidates) if stint_start_candidates else None
            stint_end_seconds = max(stint_end_candidates) if stint_end_candidates else None
            if stint_start_seconds is not None and stint_end_seconds is not None and stint_end_seconds < stint_start_seconds:
                stint_end_seconds = stint_start_seconds

            compound_series = stint_laps["Compound"].dropna().astype(str) if "Compound" in stint_laps.columns else pd.Series([], dtype=str)
            compound = compound_series[compound_series.str.strip() != ""].iloc[0] if not compound_series.empty else "-"

            if stint_end_seconds is not None:
                max_session_seconds = max(max_session_seconds, float(stint_end_seconds))
            ...
            stints.append(
                {
                    ...
                    "start_seconds": float(stint_start_seconds) if stint_start_seconds is not None else None,
                    "end_seconds": float(stint_end_seconds) if stint_end_seconds is not None else None,
                    "lap_count": len(stint_laps),
        ...
        for stint in stints[1:]:
            ...
            if stint.get("start_seconds") is not None:
                pit_seconds.append(stint["start_seconds"])
        pit_laps = sorted(set(pit_laps))
        pit_seconds = sorted(set(pit_seconds))
    ...
    if track_status is not None and not track_status.empty and laps_for_status is not None and not laps_for_status.empty:
        ...
        for _, row in status_events.iterrows():
            ...
            if segment.get("end_seconds") is not None:
                max_session_seconds = max(max_session_seconds, float(segment["end_seconds"]))

            if current is not None and current["label"] == segment["label"] and segment["start_lap"] <= current["end_lap"] + 1:
                current["end_lap"] = max(current["end_lap"], segment["end_lap"])
                if segment.get("end_seconds") is not None:
                    if current.get("end_seconds") is None:
                        current["end_seconds"] = float(segment["end_seconds"])
                    else:
                        current["end_seconds"] = max(float(current["end_seconds"]), float(segment["end_seconds"]))
                continue
    ...
    if practice_mode:
        try:
            session_time_series = session_laps["Time"].dropna() if session_laps is not None and "Time" in session_laps.columns else pd.Series([], dtype="float64")
        except Exception:
            session_time_series = pd.Series([], dtype="float64")
        if not session_time_series.empty:
            try:
                session_end_seconds = _lap_time_seconds(session_time_series.max())
            except Exception:
                session_end_seconds = None
            if session_end_seconds is not None:
                max_session_seconds = max(max_session_seconds, float(session_end_seconds))
        if max_session_seconds > 0:
            session_duration_seconds = float(max_session_seconds)
    ...
    return {
        ...
        "x_axis_mode": "time" if practice_mode else "laps",
    }
...
def _telemetry_charts(lap, gap_reference_lap=None):
    ...
    for index, (column, title, unit, transform) in enumerate(specs):
        ...
        if column == "nGear":
            ...
            y_ticks = None
            y_tick_labels = None
        elif column == "Brake" and set(y_values).issubset({0.0, 1.0}):
            y_min = 0
            y_max = 1
            y_ticks = [0, 1]
            y_tick_labels = ["0", "1"]
...
def _race_position_graph(session, year=None, gp=None, session_code="R"):
    ...
    for driver in driver_meta:
        ...
        representative_lap_number = None
        if not ordered.empty:
            first_lap_number = ordered.iloc[0].get("LapNumber")
            if pd.notna(first_lap_number):
                try:
                    representative_lap_number = int(float(first_lap_number))
                except (TypeError, ValueError):
                    representative_lap_number = None
        if representative_lap_number is None:
            try:
                fastest_lap = laps.pick_fastest()
            except Exception:
                fastest_lap = None
            if fastest_lap is not None and not getattr(fastest_lap, "empty", False):
                fastest_lap_number = fastest_lap.get("LapNumber") if hasattr(fastest_lap, "get") else None
                if pd.notna(fastest_lap_number):
                    try:
                        representative_lap_number = int(float(fastest_lap_number))
                    except (TypeError, ValueError):
                        representative_lap_number = None
        if representative_lap_number is None and not ordered.empty:
            fallback_lap_number = ordered.iloc[-1].get("LapNumber")
            if pd.notna(fallback_lap_number):
                try:
                    representative_lap_number = int(float(fallback_lap_number))
                except (TypeError, ValueError):
                    representative_lap_number = None

        href = ""
        if year is not None and gp is not None and representative_lap_number is not None:
            href_params = {
                "year": year,
                "gp": gp,
                "session": race_session_code or "R",
                "driver": driver_number,
                "lap": f"{_clean_value(driver_number)}:{representative_lap_number}",
    ...
    return {
        "title": "Sprint position changes" if race_session_code == "S" else "Race position changes",
        "lap_total": max_lap,
...
def _qualifying_overview_track_map_payload(session, phase=None, driver_options=None, phase_windows=None, session_code=None):
    if not session:
        return None
    ...
    try:
        reference_telemetry = reference_lap.get_telemetry(frequency="original").reset_index(drop=True)
    except Exception:
        reference_telemetry = None
    ...
    for sector_column, sector_label in sector_columns:
        ...
        if best_lap is not None:
            ...
            source_phase_label = _qualifying_phase_label_for_lap(session, best_lap, phase_windows=phase_windows)
            if source_phase_label:
                marker["source_phase_label"] = source_phase_label
    ...
    for index, marker in enumerate(sector_markers):
        driver_key = (
            _clean_value(marker.get("source_driver_number", "")).strip()
            or _clean_value(marker.get("source_driver_abbr", "")).strip()
            or _clean_value(marker.get("source_driver_display", "")).strip()
            or f"sector-{index}"
        )
        group = sector_group_lookup.get(driver_key)
        if group is None:
            group = {
            ...
            sector_group_lookup[driver_key] = group
            sector_groups.append(group)

        group["sectors"].append(marker)
    ...
    driver_circle_label = driver_abbr or reference_driver_number or "BEST"
    ...
    session_code = str(session_code or "").strip().upper()
    if phase_code == "ALL":
        payload["title"] = "Sprint qualifying fastest sectors" if session_code == "SQ" else "Qualifying fastest sectors"
    elif session_code == "SQ":
        payload["title"] = f"SQ{phase_code[-1]} fastest sectors"
    else:
        payload["title"] = f"{phase_code} fastest sectors"
    ...
    payload["optimal_lap_seconds"] = float(cumulative_seconds) if sector_markers else None
    payload["optimal_lap_time"] = _format_minutes_seconds(cumulative_seconds) if sector_markers else ""
    payload["sector_markers"] = sector_markers
    payload["sector_groups"] = sector_groups
...
def results():
    ...
    if session is None and data and (session_supports_position_graph or session_is_qualifying):
    ...
    return render_template(
        ...
        qualifying_driver_list_rows=qualifying_driver_list_rows,
